// Generated by CoffeeScript 1.7.1
(function() {
  (function($) {
    var component_stash, createTask, modal;
    component_stash = {};
    modal = "";
    $("a.new-task").on("click", function() {
      $("#new-task-modal-title").val("");
      $("#new-task-modal-desc").val("");
      $("#new-task-modal-duration").val("0");
      $("#new-task-modal-duration-unit").val("0");
      $("#new-task-modal-user").val("0");
      modal = $(this).attr("data-category");
      return void 0;
    });
    $("#new-task-modal-submit").on("click", function() {
      var component, s;
      component = 0;
      if (!("task" in component_stash)) {
        component = 1;
      }
      $.ajax({
        type: "POST",
        url: "/api/set/task/new",
        data: {
          component: component,
          data: {
            title: $("#new-task-modal-title").val(),
            category: modal,
            order: $(".panel[data-category='" + modal + "'] .panel-body .well").length + 1,
            desc: $("#new-task-modal-desc").val(),
            duration: $("#new-task-modal-duration").val(),
            duration_unit: $("#new-task-modal-duration-unit").val(),
            users: JSON.stringify((function() {
              var _i, _len, _ref, _results;
              _ref = $("#new-task-modal-users :selected");
              _results = [];
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                s = _ref[_i];
                _results.push($(s).val());
              }
              return _results;
            })())
          }
        },
        success: function(data) {
          console.log(data);
          if (component === 1) {
            component_stash.task = data.data.components;
          }
          return createTask();
        },
        error: function(jqXHR, textStatus, err) {
          return console.log(err);
        }
      });
      return void 0;
    });
    $(".task-list").sortable({
      axis: "y",
      containment: "parent",
      placeholder: "placeholder",
      stop: function(e, ui) {
        var t;
        return $.ajax({
          type: "POST",
          url: "/api/set/task/order",
          data: {
            order: JSON.stringify((function() {
              var _i, _len, _ref, _results;
              _ref = $(this).parent().find(".task");
              _results = [];
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                t = _ref[_i];
                _results.push($(t).attr("data-id"));
              }
              return _results;
            }).call(this))
          },
          success: function(data) {
            if (data.code !== 200) {
              return console.log("Error: " + data.code);
            }
          },
          error: function(jqXHR, textStatus, err) {
            return console.log(err);
          }
        });
      }
    }).disableSelection();
    createTask = function() {
      $("div.panel[data-category='" + modal + "'] .panel-body .task-list").append(component_stash.task.replace("{{ title }}", $("#new-task-modal-title").val()));
      $("#new-task-modal").modal("hide");
      return null;
    };
    return void 0;
  })(jQuery);

}).call(this);
