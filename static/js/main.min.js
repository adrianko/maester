...
(function() {
  ...
  (function($) {
    var component_stash, createTask, modal, taskSortable, task_list_changed, task_open;
    component_stash = {};
    modal = "";
    task_list_changed = [];
    task_open = "";
    ...
    $("#new-category-modal-submit").on("click", function() {
      var component, title;
      component = 0;
    ...
    $("#new-board-modal-submit").on("click", function() {
      var title;
      ...
      return $.ajax({
        ...
        success: function(data) {
          ...
          return $(".boards:first").append("<li class=\"list-group-item\"><a href=\"/board/" + data.data.id + "\">" + title + "</a></li>");
        },
        error: function(jqXHR, textStatus, err) {
          return console.log(err);
        }
    ...
    createTask = function(category_id, title, id) {
      var task;
      task = component_stash.task;
      task = task.replace("{{ title }}", title);
      task = task.replace("{{ id }}", id);
      $("div.panel[data-category='" + category_id + "'] .panel-body .task-list").append(task);
      return null;
    };
    return void 0;
  })(jQuery);
